...
async function xuLiGioHang(matHang){
...
async function xuLiMuaHang(matHang,id){
    const temp = popUp(matHang,id)
    await clickBuy(temp)     
}
function popUp(matHang,id){
    let thongTinSP= cuaSo_muahang.querySelector('#hienThi')
    cuaSo_muahang.classList.add('active')
    let temp={};
    matHang.forEach(function(matHang){
    if(matHang.id===id){
        temp={
